...
public final class IronPathPersistenceTest
{
    ...
    {
        FakeProfileConfig config = new FakeProfileConfig();
        config.set(IronPathPersistence.CONFIG_GROUP, IronPathPersistence.OVERRIDES_KEY,
            "broken;step.good:FORCE_COMPLETE;step.bad:NOT_A_VALUE");
        config.set(IronPathPersistence.CONFIG_GROUP, IronPathPersistence.SKIPPED_GEAR_KEY,
            ";gear.one;;gear.two;");
        IronPathPersistence persistence = new IronPathPersistence(config);
    ...
    {
        FakeProfileConfig config = new FakeProfileConfig();
        config.set(IronPathPersistence.CONFIG_GROUP, IronPathPersistence.PLAYSTYLE_KEY, "FUTURE_STYLE");
        config.set(IronPathPersistence.CONFIG_GROUP, IronPathPersistence.SESSION_LENGTH_KEY, "EXACTLY_42_MINUTES");
        config.set(IronPathPersistence.CONFIG_GROUP, IronPathPersistence.AVOID_WILDERNESS_KEY, "corrupt");
        IronPathPersistence persistence = new IronPathPersistence(config);
