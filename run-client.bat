...
if defined IRONPATH_JAVA_HOME call :consider_jdk "%IRONPATH_JAVA_HOME%"
if defined IRONPATH_JDK goto :run
...
for /d %%D in (
    ...
    if not defined IRONPATH_JDK call :consider_jdk "%%~fD"
)

for /d %%R in ("%TEMP%\ironpath-jdk11-*") do (
    for /d %%D in ("%%~fR\jdk-*") do (
        if not defined IRONPATH_JDK call :consider_jdk "%%~fD"
    )
)

if not defined IRONPATH_JDK (
    echo IronPath requires a JDK 11 or newer, but only a Java runtime was found.
    echo.
    ...
    echo run-client.bat
    echo.
    echo You can also set IRONPATH_JAVA_HOME to an existing JDK directory.
    exit /b 1
)

:run
echo Using JDK: %IRONPATH_JDK%
set "JAVA_HOME=%IRONPATH_JDK%"
...
set "IRONPATH_JDK=%CANDIDATE_JDK%"
exit /b 0
